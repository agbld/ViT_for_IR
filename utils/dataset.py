...
class TripletDatasetForImageFolderOfflineSampling(Dataset):
...
class TripletDatasetForImageFolder(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # 取 anchor
        label1 = self.train_labels[actual_index].item()
        img1 = Image.open(self.train_paths[actual_index])
        # 取 positive (確保不會取到 anchor)
        positive_index = actual_index
        while positive_index == actual_index:
    ...
    def __len__(self):
        return len(self.blister_dataset) * self.neg_ratio
    
class ContrastiveDatasetForImageFolder(Dataset):
